...
- Columnas estándar garantizadas para datos de nonces
...
# Columnas estándar para datos de nonces
...
class NonceLoader:
    ...
    def _ensure_standard_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        """Garantiza las columnas estándar en el DataFrame"""
        for col in COLUMNS:
    ...
    def load_jsonl(self, file_path, **kwargs):
        abs_path = self._abs(file_path)
        df = pd.read_json(abs_path, lines=True, **kwargs)
        return self._ensure_standard_columns(df)
    ...
    def load_log_files(self, log_dir, file_extension='*.csv', **kwargs):
        ...
        for file_path in file_paths:
            ...
            try:
                ...
                dfs.append(self._ensure_timestamp(df))
    ...
    def load_data(self, data_path, data_format='csv', **kwargs):
        ...
        Returns:
            pd.DataFrame con columnas estándar y timestamp garantizado
        """
    ...
    def load_hash_data(self, file_path: str) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Error cargando hashes desde {abs_path}: {str(e)}")
            raise
        return hashes
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Columnas estándar verificadas:", all(col in df.columns for col in COLUMNS))
        print("Columnas completas:", df.columns.tolist())
        print("Primeras filas:")
        print(df.head(2))
    except Exception as ex:
        logger.error(f"ERROR cargando datos de entrenamiento: {ex}")
    ...
    try:
        ...
        logger.error(f"ERROR cargando hashes: {ex}")
